...
class Robot(AgentTool):
        ...
        **kwargs
        ...
        if robot_type not in config_mapping:
            raise ValueError(f"Unsupported robot type: {robot_type}. "
                           f"Supported types: {list(config_mapping.keys())}")
        ...
        try:
            ...
            raise ValueError(f"Failed to create {class_name} for robot type '{robot_type}': {e}. "
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cameras": list(self.robot.config.cameras.keys()) if hasattr(self.robot, 'config') and hasattr(self.robot.config, 'cameras') else [],
        }
