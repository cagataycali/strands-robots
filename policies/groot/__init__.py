...
class Gr00tPolicy(Policy):
    ...
    def __init__(
        self, 
        data_config: Union[str, BaseDataConfig], 
        host: str = "localhost", 
        port: int = 5555,
        **kwargs
    ):
        ...
        try:
            ...
            self.data_config_name = (
                data_config if isinstance(data_config, str) 
                else type(data_config).__name__
            )
            ...
            if "zmq" in str(e).lower() or "msgpack" in str(e).lower():
                raise ImportError(
                    f"GR00T dependencies not available: {e}. "
                    f"Please install: pip install msgpack pyzmq"
                ) from e
            else:
